...
class CHE_Record:
    ...
    def record(self, chess, x, y):
        ...
        if chess.kind < 7:
            self.QIPU += [[self.Board.state.state_no,
                           [chess.No, 32, 0, 9 - chess.y, 8 - chess.x, 9 - y, 8 - x, 0, hui_he * 2,
                            0]]]  # 真实记谱.che格式
            for i in range(10):
            ...
            hui_he += 1
            self.QIPU += [[self.Board.state.state_no,
                           [chess.No, 32, 1, 9 - chess.y, 8 - chess.x, 9 - y, 8 - x, 0, hui_he * 2 - 1,
                            0]]]  # 真实记谱.che格式
            for i in range(10):
                if self.Board.state[chess.x, i][1] == chess.kind and i != chess.y:
                    if i > chess.y:
                        TEXT = rjust(str(hui_he), 3) + u'. \u524D' + TEXT
                        break
                    else:
                        TEXT = rjust(str(hui_he), 3) + u'. \u540E' + TEXT
                        break
            else:
                TEXT = rjust(str(hui_he), 3) + '. ' + TEXT
    ...
    def qi_pu(self, listbox, Board, chess, x, y):
        ...
        listbox.see(END)
        global process
        process += [[Board.state.getno(),Board.state.getkind()]]
