...
class Bishop(Piece):
    def legal_move(self, x, y):
        if abs(self.chess.x - x) == 2 and abs(self.chess.y - y) == 2 and \
                        self.Board.state[(self.chess.x + x) / 2, (self.chess.y + y) / 2][0] == -1:  # 象走田，不塞象心
...
class Ambassador(Piece):
    def legal_move(self, x, y):
        if abs(self.chess.x - x) == 1 and abs(self.chess.y - y) == 1:  # 士斜走
            if 3 <= x <= 5 and ((self.chess.kind < 7 and 0 <= y <= 2)
                                or (self.chess.kind > 6 and 7 <= y <= 9)):  # 不出九宫
                return True
        return False
...
class King(Piece):
    def legal_move(self, x, y):
        if (abs(self.chess.x - x) == 1 and self.chess.y == y) or (
                        abs(self.chess.y - y) == 1 and self.chess.x == x):  # 帅(将)直走一格
            if 3 <= x <= 5 and ((self.chess.kind < 7 and 0 <= y <= 2)
                                or (self.chess.kind > 6 and 7 <= y <= 9)):  # 不出九宫
                return True
        return False
...
class Pawn(Piece):
    def legal_move(self, x, y):
        if self.chess.kind < 7:
            if (self.chess.y < 5 and self.chess.x == x and y - self.chess.y == 1) or (self.chess.y > 4 and (
                        (self.chess.x == x and y - self.chess.y == 1) or (
                            self.chess.y == y and abs(self.chess.x - x) == 1))):
                return True
        if self.chess.kind > 6:
            if (self.chess.y > 4 and self.chess.x == x and self.chess.y - y == 1) or (self.chess.y < 5 and (
                        (self.chess.x == x and self.chess.y - y == 1) or (
                            self.chess.y == y and abs(self.chess.x - x) == 1))):
                return True
        return False
