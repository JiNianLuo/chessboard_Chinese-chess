...
class appArb:
    ...
    def move(self, event):
        ...
        if self.choose and x > 8 and (self.choosed_chess.kind != 7 * (x - 9) + (y - 2)):  # 有选中的棋子,并且再次点击位置与棋子位置不同
            ...
            self.choosed_chess.choose()
	    return
        if self.choose and self.choosed_chess.x > 8 and x < 9 and self.right_put(self.choosed_chess,x,y):  # 选中棋子，并且移到棋盘内
	...
	if not self.choose or (self.choose and self.choosed_chess.x < 9):
		if x < 9:
        	 if not self.choose and self.Board.state[x, y][0] != -1:#无选中的棋子时选中棋盘内点击位置的棋子
